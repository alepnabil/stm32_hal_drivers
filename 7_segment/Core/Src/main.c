...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOC, PC5_Pin|PC6_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : PC5_Pin PC6_Pin */
  GPIO_InitStruct.Pin = PC5_Pin|PC6_Pin;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	  if( GPIO_Pin ==  PC1_Pin)
	  {
		  ...
		  if(HAL_GPIO_ReadPin(PC1_GPIO_Port,PC1_Pin)==GPIO_PIN_RESET)
		  {
			  // to make number 1
